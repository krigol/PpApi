...
namespace BeerPal.Controllers
{
    public class ECommerceController : Controller
    {
        ...
        public ECommerceController(IPayPalApiHelperService payPalApiHelperService)
        {
                ...
                var payment = new Payment
                {
                    ...
                    transactions = new List<Transaction>
                    {
                        new Transaction
                        {
                            ...
                            {
                                currency = "USD",
                                total = (order.Total/100M).ToString(CultureInfo.InvariantCulture), // PayPal expects string amounts, eg. "20.00",
                                details = new Details()
                                {
                                    subtotal = (order.Subtotal/100M).ToString(CultureInfo.InvariantCulture),
                                    shipping = (order.Shipping/100M).ToString(CultureInfo.InvariantCulture),
                                    tax = (order.Tax/100M).ToString(CultureInfo.InvariantCulture)                                    
                                }
                            },                            
                            ...
                            {                                
                                items =
                                    order.OrderItems.Select(x => new Item()
                                    {
                                        ...
                                        price = (x.Price/100M).ToString(CultureInfo.InvariantCulture), // PayPal expects string amounts, eg. "20.00"
                                    }).ToList()
                            }
                        }
                    },
